	...
	for (;;) {
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0)
			...
			if(fifo8_status(&keyfifo) != 0) {
				...
				io_sti();
				sprintf(s, "%02X", i);
				boxfill8(binfo->vram, binfo->scrnx, COL8_008484, 0, 15, 15, 31);
				putfonts8_asc(binfo->vram, binfo->scrnx, 0, 15, COL8_FFFFFF, s);
			} else if(fifo8_status(&mousefifo) != 0) {
				int i = fifo8_get(&mousefifo);
				io_sti();
				sprintf(s, "%02X", i);
				boxfill8(binfo->vram, binfo->scrnx, COL8_008484, 0, 31, 15, 47);
				putfonts8_asc(binfo->vram, binfo->scrnx, 0, 31, COL8_FFFFFF, s);
			}
		}
	}
}
